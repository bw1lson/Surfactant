...
def _collect_symbol_versions(filename: str) -> dict[str, Any]:
    ...
    *minimum* library version the binary can run against, which is useful for
    ...
    """
    try:
        # Imported lazily; pyelftools is already a transitive dependency of angr.
        from elftools.common.exceptions import ELFError
        from elftools.elf.elffile import ELFFile
        from elftools.elf.gnuversions import GNUVerNeedSection
    except ImportError:  # pragma: no cover - pyelftools should always be present
        return {}
...
def establish_relationships(
    ...
    for imp in imported:
        ...
        for exporter_uuid in export_index.get(name, set()):
            if exporter_uuid == software.UUID:
                continue
            if linked_libs:
                exporter = sbom.get_software(exporter_uuid) if hasattr(sbom, "get_software") else None
    ...
    for exporter_uuid, symbols in matches.items():
        ...
        logger.debug(
            f"angr_expanded: {software.UUID} Uses {exporter_uuid} "
            f"({len(symbols)} matched symbols)"
        )
