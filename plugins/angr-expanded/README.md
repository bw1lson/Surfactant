...
- **Static control-flow graph summary** (optional, off by default — see below)
  - `cfg.functionCount`
  - `cfg.recoveredFunctions` — names of functions recovered by `CFGFast`
  - `cfg.notableApiReferences` — security-relevant APIs referenced by the code
    (e.g. `system`, `execve`, `dlopen`, `VirtualProtect`, `LoadLibraryA`)
  - `cfg.callGraph` — per-function `{"calls": [...], "calledBy": [...]}` map from
    angr's recovered call graph (which functions each function calls, and which
    functions call it). Calls to library functions appear by their imported name
    (e.g. `main` → `printf`).
...
## Enabling the optional CFG analysis

Building a control-flow graph with `CFGFast` can be slow on large binaries, so it
is **disabled by default**. Enable it via Surfactant settings:

```bash
surfactant config angr_expanded.enable_cfg true
```

Disable it again with:

```bash
surfactant config angr_expanded.enable_cfg false
```
